...
def create_combinations(types):
    """Create all possible combinations of three types."""
    return [f"{', '.join(p)}" for p in itertools.product(types, repeat=3)]

def generate_code(types):
    """Generate variant, vector, and params."""
    type_combinations = create_combinations(types)
